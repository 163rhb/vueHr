        ...
        methods: {
            ...
            deleteSalary(data) {
                this.$confirm('此操作将删除【' + data.name + '】账套，是否继续？', '提示', {
                    ...
                    this.deleteRequst("/salary/sob/" + data.id).then(resp => {
                        if (resp) {
                            this.initSalaries();
                        }
                    })
            ...
            nextStep() {
                if (this.activeItemIndex == 10) {
                    if (this.salary.id) {
                        this.putRequst("/salary/sob/", this.salary).then(resp=>{
                            if (resp) {
                                this.initSalaries();
                                this.dialogVisible = false;
                        ...
                        this.postRequst("/salary/sob/", this.salary).then(resp => {
                            if (resp) {
                                this.initSalaries();
                                this.dialogVisible = false;
                                this.activeItemIndex=0
            ...
            initSalaries() {
                this.getRequst("/salary/sob/").then(resp => {
                    if (resp) {
                        this.salaries = resp;
                    }
                })
